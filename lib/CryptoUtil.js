...
export class CryptoUtil {
	...
	static keyBytes = crypto.randomBytes(256 / 8);
	// alg: RS256 ***
	...
	static async symmetricEncrypt({ data }) {
		const nonce = 128;
		const iv = await this.randomBytes(nonce);
		const algorithm = "aes-256-gcm";
		const cipher = crypto.createCipheriv(algorithm, this.keyBytes, iv);
	...
	static asymmetricEncrypt() {
		console.log('Encrypting the AES Random Key.');

		const paddingOptions = {
			oaepHash: 'sha256',
			mgf1Hash: 'sha256',
			label: Buffer.alloc(0)
		};

		const encryptedKey = crypto.publicEncrypt({
			key: this.getCertificateObject().publicKey,
	...
	static getCertificateObject(){
		return new crypto.X509Certificate(fs.readFileSync(process.env.CRYPTO_ENCRYPT__ENCRYPT_CERT_PATH));

	}

	static async getCertThumprintHash() {
		const x509 = this.getCertificateObject();
